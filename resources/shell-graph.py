...
from App import models, types, controllers
...
def method1(bms):
    ...
    # 
    start = time()
    data = controllers.WordGraphBuilder(similarity.document_ids, similarity.similarity_matrix).build()
...
def method2(new, old):
    ...
    print('end sleep')
    x = controllers.GraphNewNodes(document_ids, similarity_matrix, old_document_ids, intersect_similarity)
    start = time()
    x.run()
