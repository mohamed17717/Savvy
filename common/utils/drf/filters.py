...
class FullTextSearchFilter(SearchFilter):
    ...
    def get_raw_search_terms(self, request, search_param=None):
        ...
        def to_raw(q):
            ...
            def multiple_query(i):
                return parentheses(and_join(map(quote, i.split(" "))))

            q = " ".join(query)
            q = q.split("|")
            q = or_join([multiple_query(i) if is_multiple(i) else quote(i) for i in q])
            return q
    ...
    def filter_queryset(self, request, queryset, view, distinct=True):
        ...
        search_terms = self.get_raw_search_terms(
            request, search_param=getattr(view, "search_param", "search")
        )
